...
public class Part3 extends JFrame implements GLEventListener{
	...
	public void rasterize(int i,Vector A,Vector B,Vector C, float x1,float x2,float x3,float y1,float y2,float y3,float z1,float z2,float z3){
		...
		float beta = ((y1 - y3) * x + (x3 - x1) * y + x1 * y3 - x3
				* y1)/ ((y1 - y3) * x2 + (x3 - x1) * y2 + x1 * y3 - x3*y1);
		...
		if (gamma >= 0 && gamma <= 1 && beta >= 0 && beta <= 1
			...
			data[j + 0] = (float) Math.pow(alpha*A.getColor()[0]+beta*B.getColor()[0]+gamma*C.getColor()[0],1/2.2); // red
			data[j + 1] = (float) Math.pow(alpha*A.getColor()[1]+beta*B.getColor()[1]+gamma*C.getColor()[1],1/2.2); // green
			data[j + 2] = (float) Math.pow(alpha*A.getColor()[2]+beta*B.getColor()[2]+gamma*C.getColor()[2],1/2.2); // blue
			}
		}
